= SCE geisa-imx93 Yocto Platform Developer Guide
...
The part specific to SCE's product is stored on a private git server, provided
...
Then, connect to https://g1.sfl.io[gerrit] and authenticate using your
...
For instance, for SCE's yocto-build project:

  $ cd my_project_dir/
  $ repo init -u ssh://g1.sfl.io:29419/SCE/geisa-imx93/repo-manifest
  $ repo sync

Once the sync is completed, you should see a git repository named `yocto-build`,
within which all the Yocto layers were fetched under the `yocto-build/sources`
sub-directory.
...
* Install repo if it is not already done:
...
The build is started by running the following command:
  ...
  $ ./build.sh -i geisa-imx93-image -m geisa-imx93

You can also pass custom BitBake commands using the `--` separator:

  $ ./build.sh -i geisa-imx93-image -m geisa-imx93 -- bitbake -c clean package_name
...
A debug image is available, with additional tools and relaxed security features
available for development. It can be built using cqfd, or manually:

  $ cqfd -b debug
  $ ./build.sh -i geisa-imx93-dbg-image -m geisa-imx93
...
== Running the debug image in QEMU

You can run the built debug image in QEMU virtual machine with the following command:
...
To launch the flash execute:
    ...
    $ sudo ./uuu -b emmc_all geisa-imx93-image-geisa-imx93.rootfs.wic.zst
...
To launch the flash execute:
    ...
    $ sudo ./uuu -b emmc_all geisa-imx93-image-geisa-imx93-evk.rootfs.wic.zst
